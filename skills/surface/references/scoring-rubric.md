...
**Signal applicability:** Within an applicable web surface, activate optional checks only when their capability is detected: API Catalog for a public HTTP API, MCP metadata for a remote MCP server, OAuth metadata for protected resources, Agent Skills discovery for published skills, WebMCP for page-scoped agent interactions, commerce manifests for commerce, and Web Bot Auth for signed bot identity. Record other signals as not applicable rather than passes or failures.

**Semantic validation:** A file or HTTP 200 is insufficient evidence. Parse the artifact, reject HTML/login/placeholder bodies, resolve its advertised links, and safely sample the capability. For example, hash linked Agent Skills against their published digests and read advertised MCP resources to confirm non-empty content and accurate MIME types.
...
**Key files:** AGENTS.md, CLAUDE.md (if present), .cursor/rules/\*.mdc, .github/copilot-instructions.md, .windsurf/rules/
