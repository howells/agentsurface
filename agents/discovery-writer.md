...
description: Generate llms.txt, llms-full.txt, AGENTS.md discovery, robots-ai.txt, JSON-LD, and content negotiation for agent crawling
model: sonnet
...
Emit agent-discoverable artifacts: llms.txt (curated index <5k tokens), AGENTS.md (agent boundaries + commands), robots-ai.txt (crawler permissions), JSON-LD structured data (schema.org), and content negotiation middleware (Accept: text/markdown).
...
- Content negotiation: Return Markdown + Vary header on Accept: text/markdown

## Mission

Guide LLM agents to your docs, APIs, and boundaries through machine-readable discovery files. No guessing; every agent should find what it needs within 3 requests.
...
4. **Create/update robots.txt**:
   ...
   ```
   - Explicitly allow AI training bots (GPTBot, ClaudeBot, anthropic-ai)
   - Block training bots only if requested
...
7. **Implement content negotiation** (optional, for Next.js/Express):
   - Add middleware to handle `Accept: text/markdown`:
     ...
     ```
   - Vary header is critical for CDN caching
...
9. **Quality checks**:
   ...
   - robots-ai.txt is valid YAML/TOML
