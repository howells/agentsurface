...
const ROLES = "type-display / type-heading / type-lead / type-body / type-small / type-data";
...
function problem(base) {
  ...
  if (FONT_FAMILY.test(base)) {return `inline font family "${base}" - type-data is the mono role`;}
  if (TRACKING.test(base)) {return `inline tracking "${base}" - the role sets tracking`;}
  if (LEADING.test(base)) {return `inline line-height "${base}" - the role sets leading`;}
  if (CASE.test(base)) {return `inline casing "${base}" - casing belongs to a role`;}
  return null;
}
...
function* files(dir) {
  for (const name of readdirSync(dir)) {
    ...
    else if (/\.(tsx|jsx)$/u.test(name)) {yield path;}
  }
}
    ...
    for (const match of source.matchAll(CLASS_STRING)) {
      ...
      for (const chunk of chunks) {
        for (const token of chunk.split(/\s+/u).filter(Boolean)) {
          ...
          if (message) {violations.push(`${relative(process.cwd(), file)}:${line} ${message}`);}
        }
      }
    }
  }
