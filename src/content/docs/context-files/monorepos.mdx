...
├── CLAUDE.md                      # Claude Code overlay (optional; applies to all packages)
...
packages/web: Frontend (Next.js 16)
...
There is no single cross-tool precedence ladder - each tool resolves its own files independently, and the rules don't merge:

- **AGENTS.md itself**: nested files win over the root. `packages/api/AGENTS.md` overrides `AGENTS.md` (root) for anything under `packages/api/`.
- **Claude Code** (v2.1.277+): reads `AGENTS.md` directly when no `CLAUDE.md` exists anywhere from the working directory up to the repo root; if one does exist, Claude Code reads that instead unless it starts with a literal `@AGENTS.md` import - see [CLAUDE.md](/docs/context-files/claude-md) for the exact rule.
- **Cursor**: reads `AGENTS.md` natively (nested wins) and layers `.cursor/rules/*.mdc` on top for glob-scoped detail - see [Cursor and Copilot](/docs/context-files/cursor-and-copilot).
- **GitHub Copilot**: reads `AGENTS.md` directly (coding agent, code review) and separately reads `.github/copilot-instructions.md` plus path-scoped `.github/instructions/*.instructions.md`.

**Example:** For API server conventions, a package-scoped `packages/api/AGENTS.md` is what every tool prefers over the root file for that package - that part is shared. What differs is whether a given tool also reads a tool-specific file (`CLAUDE.md`, `.cursor/rules/`, `.github/instructions/`) alongside it, and each tool's own docs are the source of truth for that.
