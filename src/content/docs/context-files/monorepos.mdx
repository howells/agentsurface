...
- **Claude Code**: loads `CLAUDE.md` when one exists anywhere from the working directory up to the repo root, and `AGENTS.md` only when no `CLAUDE.md` exists; combine the two with the `@AGENTS.md` import - see [CLAUDE.md](/docs/context-files/claude-md) for the exact rule.
