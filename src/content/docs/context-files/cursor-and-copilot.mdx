...
Tool-specific overrides layered on AGENTS.md. Cursor reads `AGENTS.md` natively (root and nested, closest file wins) and also reads `.cursor/rules/*.mdc` files for glob-scoped rules; GitHub Copilot reads `.github/copilot-instructions.md` for repo-wide guidance and `.github/instructions/*.instructions.md` for path-scoped guidance via `applyTo` glob frontmatter. Keep tool-specific files minimal, avoid duplication, link to shared conventions in AGENTS.md.

- **Cursor**: reads `AGENTS.md` natively (root and nested, closest file wins), with `.cursor/rules/*.mdc` adding glob-scoped detail on top
- **Copilot instructions**: `.github/copilot-instructions.md` (repo-wide) plus `.github/instructions/*.instructions.md` (path-scoped via `applyTo`)
- **Copilot coding agent and code review**: read `AGENTS.md` directly (GA 2026)
- **Glob scoping**: apply rules per file pattern in both tools
...
Cursor reads `AGENTS.md` directly - root and nested subdirectories, with the closest file to the current context winning - and layers `.cursor/rules/*.mdc` files on top for glob-scoped detail `AGENTS.md` doesn't need to carry everywhere. GitHub Copilot reads `AGENTS.md` directly too, for its coding agent and code review features (GA 2026), and additionally reads `.github/copilot-instructions.md` (repo-wide) and `.github/instructions/*.instructions.md` (path-scoped, via `applyTo` glob frontmatter). Keep all of these thin and focused: reference shared conventions from AGENTS.md, add tool-only guidance.

## Cursor Rules (.cursor/rules)

Use `.cursor/rules/*.mdc` files when you need guidance scoped to a specific glob that doesn't belong in the shared `AGENTS.md`. Cursor supports multiple rule files scoped by glob patterns. Each file is a `.mdc` (Markdown Cursor) file that specifies where it applies. The older single-file `.cursorrules` format is deprecated in Cursor's own docs - migrate it to `.cursor/rules/*.mdc` or to `AGENTS.md`.
...
GitHub Copilot reads `.github/copilot-instructions.md` for repo-wide guidance. For path-scoped guidance, add `.github/instructions/*.instructions.md` files with `applyTo` glob frontmatter - the same idea as Cursor's per-file globs:
...
├── CLAUDE.md       (optional; starts with `@AGENTS.md` import, add Claude-only hooks)
...
2. If a CLAUDE.md exists, leave its `@AGENTS.md` import in place - do not restate the change in prose
...
That single line pulls in the whole AGENTS.md file, Testing section included - nothing to restate. Add Claude-only lines below the import only if Claude needs something no other tool should see.
...
- Tests: src/\*_/_.test.ts (Vitest)
- Run: pnpm test
- Structure: describe, it, expect
- Test behavior, not implementation
```

All files reference or import AGENTS.md as source of truth. No duplication.
...
- ([Cursor rules docs](https://docs.cursor.com/context/rules)) - includes AGENTS.md support
- ([GitHub Copilot repository instructions](https://docs.github.com/en/copilot/how-tos/configure-custom-instructions/add-repository-instructions)) - repo-wide and path-scoped instructions
