...
Context files go stale. Commands, directories and conventions change. Agents that follow outdated instructions fail silently - running non-existent commands, trying to access deleted files, applying superseded conventions.
...
# Check line count (warn if over 370)
lines=$(wc -l < AGENTS.md)
if [ "$lines" -gt 370 ]; then
  echo "AGENTS.md is $lines lines (recommend under 370)"
  exit 1
fi
...
Run the `/surface` skill's scorecard against context files from an agent session:
...
#   Score: 2/3
#   Issues:
#     - AGENTS.md: 425 lines (recommend under 370)
#     - CLAUDE.md: Last verified 2026-02-01 (>3 months stale)
...
This is a skill invocation inside an agent session, not a CLI binary - there is nothing here for a CI job to shell out to. Run it on a cadence (monthly or quarterly) or after a change that touches commands, tech stack, or boundaries, and treat a low score as a prompt to investigate and fix, not a merge gate. The shell scripts above are what actually belongs in CI: they run as ordinary commands and can fail a build.
...
- No changes needed in CLAUDE.md - it imports AGENTS.md, so it picked up the update automatically
...
Blocks merge if the shell scripts fail. The `/surface` scorecard from the previous section runs separately, in an agent session on a cadence - it has no CLI form for CI to call, and its purpose is diagnostic, not a merge gate.
...
- [ ] CLAUDE.md, if present, imports AGENTS.md with `@AGENTS.md` (no duplication)
