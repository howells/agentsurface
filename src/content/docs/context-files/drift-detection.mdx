...
Use `agentify` CLI to audit context files:

```bash
# Command-line audit
agentify score --dimension=context
...
jobs:
  context-files:
    ...
    steps:
      ...
      - run: npm install -g agentify
      - run: agentify score --dimension=context --fail-below 2
```
...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Run agentify audit
        run: |
          npm install -g agentify
          agentify score --dimension=context --fail-below 2
```
