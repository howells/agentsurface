...
Field masks should work on every read command, including single-resource reads. Getting a single resource should also support `--fields`:
  ...
  return fields.reduce((acc, field) => {
    ...
    if (parts.length === 1) {
      ...
      if (nested && typeof nested === "object") {
        const projected = applyFieldMask(nested as Record<string, unknown>, [tail.join(".")]);
...
List commands that are paginated by default should offer `--all` (sometimes named `--page-all`) to fetch all pages automatically. Without this, an agent must manually iterate pages by tracking cursors - which requires extra tool calls and more complex prompting.
...
`--all` concatenates everything before outputting, which means the agent waits for the full dataset before it can process any records. For large datasets, this is slow and still consumes peak memory.
...
The most durable context window protection is documentation that teaches agents to use the flags correctly before they need to.
...
| 2     | Field masks on all read commands. Pagination with `--all` or equivalent.                                                                                    |
...
- [ ] `--all` flag fetches all pages automatically
