...
A delimiter, or data-tagging, pattern defends against this by wrapping API response content in structured delimiters that signal to the LLM: "this is data, not instructions."
...
For higher-assurance contexts, run returned text through your approved prompt-injection or content-filtering layer before returning it to the agent. Document the scanner, policy version, and action taken on failure. Google Cloud's Model Armor is one such layer - a standalone REST API that screens prompts and responses independently for content-safety categories, prompt-injection and jailbreak attempts, sensitive-data exposure, and malicious URLs, running in "inspect only" or "inspect and block" mode. It is a request/response screening service you put in front of the CLI's data path, separate from the delimiter pattern above.
