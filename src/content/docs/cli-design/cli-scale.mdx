...
The Agent DX CLI Scale scores CLIs on seven axes (0-3 each, 0-21 total): machine-readable output, raw payload input, schema introspection, context window discipline, input hardening, safety rails, and agent knowledge packaging. The score maps to readiness tiers from Human-only (0-5) to Agent-first (16-21). This framework quantifies gaps and helps prioritize work.
...
| 3     | Dry-run plus response screening (for example, running API responses through a screening service such as Google Cloud Model Armor) to catch prompt injection embedded in API data. The full request→response loop is defended. |
...
3. **Test the edge cases.** Does JSON output work for errors and successes? Does `--dry-run` exist for delete and create?
...
The Stripe CLI (`stripe`) is a widely-used API CLI that scores well on several axes and illustrates the tradeoffs. Stripe has shipped dedicated agent tooling, which moves three of the seven axes.

| Axis                      | Score  | Notes                                                                                                                                                                                                                                                                                                                                                                      |
| ------------------------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Machine-Readable Output   | 2      | `stripe login --non-interactive` auto-activates and prints JSON once stdin isn't a terminal; `post`/`get`/`delete` return the Stripe API's native JSON by default; `pay` requires `--agent`/`--json` for scripted use. NDJSON streaming for paginated list output is not documented, so this stops short of a 3.                                                           |
...
| Safety Rails              | 1      | `--dry-run` exists on `stripe pay` (the money-moving command) with a `confirmation_required` JSON contract gating `--yes`, so an agent can validate a transfer before it sends money; the generic `post`/`delete` mutation commands have no `--dry-run`, only an interactive confirm prompt (`--confirm` to skip it) - coverage is one command, not all mutating commands. |
| Agent Knowledge Packaging | 2      | `stripe agent setup` detects Claude Code, Codex, and Cursor and installs a Stripe plugin, skills, and an MCP server; `stripe projects init` and `stripe projects llm-context` generate a project `AGENTS.md`.                                                                                                                                                              |
| **Total**                 | **11** | **Agent-ready**                                                                                                                                                                                                                                                                                                                                                            |

Source for the current behavior: https://docs.stripe.com/stripe-cli/reference (fetched 2026-09-25).

Score of 11 means: an agent can use the Stripe CLI with structured output and packaged agent knowledge already in place, but still cannot do runtime schema discovery, still gets full API objects with no field-selection flag, and only the highest-stakes mutating command (`pay`) has a validate-before-acting path.

Reaching Agent-first (16+) would require: runtime schema resolution (Schema Introspection), a `--fields` flag on read commands (Context Window Discipline), comprehensive input hardening, and `--dry-run` extended from `pay` to the generic `post`/`delete` mutation commands.

## Using the Scale to Prioritize Work

Not all axes have equal impact. The ordering below reflects which improvements deliver the most agent-reliability per unit of implementation effort:
...
3. **Schema Introspection (Axis 3)** - enables agents to self-discover without pre-loaded docs. Medium effort, high impact.
4. **Context Window Discipline (Axis 4)** - `--fields` is low effort; streaming takes more work.
5. **Raw Payload Input (Axis 2)** - high impact for API-backed CLIs; relatively simple to add.
