...
Standard input validation catches the human failure modes. Agent-specific input hardening catches the hallucination patterns that standard validation misses entirely.

<Callout type="warn">
  The security posture for agent-facing CLIs must be: **the agent is not a trusted operator**. Even
  when an agent is acting on behalf of an authorized user, it can be manipulated through prompt
  injection (see [Safety Rails](./safety-rails)) or simply hallucinate inputs that violate intended
...
- [Raw Payload Input](./raw-payload-input) - validating payload content and flags
