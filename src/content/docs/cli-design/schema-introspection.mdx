...
SKILL.md is a structured capability declaration that pairs with the CLI binary to give agents a concise, versioned description of what the CLI can do and the invariants they must follow when using it.

A SKILL.md file lives in the repository root (or alongside the binary) and follows this structure:
...
Auto-generate SKILL.md from your CLI's describe output as part of your release pipeline:

```bash
# Generate SKILL.md from live schema
...
- [ ] SKILL.md file ships with the CLI in the repository root
- [ ] SKILL.md frontmatter includes `schema_command` pointing to the introspection command
...
- [Agent Knowledge Packaging](./agent-knowledge-packaging) — SKILL.md and context file conventions
