...
Every CLI that wraps an API faces the translation problem: the underlying API accepts a rich JSON payload, but the CLI exposes it through bespoke flags that map - imperfectly - to that payload. Humans work around the mismatch; agents propagate it into wrong invocations.
...
Accepting JSON from stdin makes agent pipelines composable. Follow these conventions:
