...
The `SKILL.md` folder shape - YAML frontmatter with `name`/`description`, a Markdown body, optional `scripts/`, `references/`, and `assets/` - is covered on [Agent Skills](/docs/discovery/agent-skills); this section covers only what a CLI adds to that base shape.

Skill folders are read off disk at conversation start, loaded at runtime by agent frameworks as a skill tool surface, and attached server-side by hosted model platforms to their managed execution environments - see [Agent Skills](/docs/discovery/agent-skills) for the consumption models. Packaging a CLI's knowledge as skills therefore pays off in three places at once, without a translation layer.

A CLI skill extends the base frontmatter with fields that point at runtime introspection instead of a static description - see [Schema Introspection](./schema-introspection#skill-metadata-for-clis) for the `schema_command`/`auth`/`capabilities` fields and a worked example. Keep the base `name` and `description` valid on their own for agents that only understand the public Agent Skills format; treat the extension fields as additive.

The skill body should encode preview-then-execute pairs, not prose warnings:
...
# Step 3: execute only if the preview is acceptable
mytool users delete --status suspended --yes --json
```

Field mask usage belongs in the skill body too - see [Context Window Discipline](./context-window-discipline) for the `--fields` mechanics this invariant depends on.
...
````markdown
---
name: mytool-dns-records
description: DNS record commands, propagation timing, and invariants for mytool
...
These invariants reduce the need for the agent to reason about safety on every call.
...
| 2     | Agent Skills covering per-command or per-API-surface workflows and invariants.                                                                                                                 |
...
- [Schema Introspection](./schema-introspection) - CLI schema commands and optional skill metadata
