...
const Scorecard = z.object({
  ...
  dimensions: z.array(Dimension).length(11),
  findingClusters: z.array(FindingCluster),
  overall: z.object({
    rawScore: z.number().int().min(0).max(33),
    maxApplicable: z.number().int().min(1).max(33),
    scaledScore: z
      .number()
      .int()
      .min(0)
...
```json
{
  ...
  "dimensions": [
    ...
    },
    {
      ...
      "notes": "RFC 9457 implemented. is_retriable present but not on all error types. Missing suggestions array and doc_uri."
    },
    {
      "name": "tool-design",
      "score": 1,
