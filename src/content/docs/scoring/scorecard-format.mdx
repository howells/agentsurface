...
description: JSON schema and TypeScript Zod schema for scorecards
...
Agent Surface emits structured scorecards as JSON. This page documents the schema, provides a TypeScript/Zod definition, and shows a worked example.
...
const Finding = z.object({
  id: z.string().describe('Stable ID other Finding arrays reference, e.g. "mcp-not-implemented"'),
...
const OverallRating = z.enum(["human-only", "agent-tolerant", "agent-ready", "agent-first"]);
...
const Scorecard = z.object({
  ...
  overall: z.object({
    rawScore: z.number().int().min(0).max(30),
    maxApplicable: z.number().int().min(1).max(30),
    scaledScore: z
      .number()
      .int()
      .min(0)
      .max(30)
      .describe("Raw score scaled to 30 after excluding N/A dimensions"),
...
```json
{
  ...
  "project": {
    "name": "acme-widgets",
    "repository": "https://github.com/acme/widgets-api",
  ...
  "dimensions": [
      ...
      "evidence": [
        {
          ...
          "result": "40 endpoints, operationId on 40 (100%), descriptions 30-50 words"
        },
        {
          "type": "grep",
    ...
    },
    {
      ...
      "findings": ["discovery-llms-txt-missing"],
    ...
    },
    {
      ...
      "findings": [],
      "notes": "OAuth 2.1 M2M with scoped tokens. Token exchange (RFC 8693) not applicable to the advertised flows."
    },
    {
      "name": "error-handling",
      "score": 2,
      ...
      "evidence": [
        {
          ...
          "result": "Returns RFC 9457 Problem Details: { type, title, status, detail, instance }"
        },
        {
          "type": "grep",
          "description": "is_retriable field",
          "location": "grep -r 'is_retriable\\|isRetriable' src/",
          "result": "is_retriable boolean present on all error types"
        },
        {
          "type": "command",
      ...
      "findings": ["error-missing-doc-uri"],
      "notes": "RFC 9457 implemented with complete is_retriable coverage and a suggestions array. Missing doc_uri for level 3."
    },
    {
      "name": "tool-design",
      "score": 1,
      ...
      "findings": ["tools-weak-descriptions"],
    ...
    },
    {
      ...
      "evidence": [
        ...
        {
          "type": "grep",
          "description": "CLAUDE.md imports AGENTS.md",
          "location": "grep -c '^@AGENTS.md' CLAUDE.md",
          "result": "No CLAUDE.md present - not required for a top score"
        },
        {
          "type": "grep",
      ...
      "findings": [],
      "notes": "Good AGENTS.md. No CLAUDE.md or Cursor rules present - neither is required; add one only if that tool needs something AGENTS.md can't give it."
    },
    {
      "name": "testing",
      "score": 1,
      ...
      "findings": ["testing-error-recovery-missing"],
    ...
    },
    {
      ...
      "evidence": [],
      "findings": [],
      "notes": "N/A: Project does not expose a searchable knowledge or retrieval surface"
    }
  ],
  "findingClusters": [
    {
      "name": "Agent tool discoverability",
      "rationale": "Agents struggle to find and understand tools because tool descriptions are terse, no MCP server exists, and llms.txt is missing.",
      "findings": [
        {
          "id": "tools-weak-descriptions",
    ...
    },
    {
      ...
      "rationale": "Agents cannot link an error back to documentation, and testing does not cover recovery flows.",
      "findings": [
        {
          "id": "error-missing-doc-uri",
          "what": "src/middleware/errors.ts: Problem Details responses omit doc_uri",
          "why": "Agents use doc_uri to look up the relevant error-reference page without guessing a URL.",
          "fix": "Add a doc_uri field to the Problem Details response, pointing at the matching error-reference page.",
        ...
        {
          "id": "testing-error-recovery-missing",
      ...
      "suggestedApproach": "Add doc_uri to the error schema (1 hour). Write recovery tests (2 hours). Total: 3 hours.",
      "dependencies": []
    }
  ],
  "overall": {
  ...
  "trend": {
    "previousDate": "2026-05-01T10:00:00Z",
    "previousScore": 8,
    "delta": 3,
    "status": "improved"
  }
