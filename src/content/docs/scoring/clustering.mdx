...
**Findings:** No RFC 9457 + missing is_retriable + no suggestions array + no error tests
...
**Findings:** No AGENTS.md + missing context boundaries + no llms.txt
...
- Effort: 6.5 hours | Impact: +4 dimensions | Ratio: Excellent

**Cluster 2: Machine-readable auth**

- OAuth 2.1 Client Credentials, scoped tokens, and JWT validation (↑Authentication 1→2)
- Tested renewal, revocation, and wrong-user/wrong-resource rejection (↑Authentication 2→3)
- Effort: 5 hours | Impact: +1 dimension (fully) | Ratio: Good

**Cluster 3: Error recovery**

- RFC 9457 Problem Details (↑Error Handling 0→2)
...
```
PHASE 1: Agent tool discoverability (Week 1, 6.5 hours)
...
PHASE 2: Machine-readable auth (Week 1, 5 hours)
├─ Task 2.1: Implement OAuth 2.1 Client Credentials [2h]
├─ Task 2.2: Add token scoping and JWT validation [2h]
├─ Task 2.3: Test renewal, revocation, wrong-resource rejection [1h]

PHASE 3: Error recovery (Week 2, 3 hours)
├─ Task 3.1: Refactor to RFC 9457 format [1.5h]
...
Expected improvement: 6/30 → 20/30 (Human-only → Agent-ready)
```
...
Ratio (impact/effort):  0.67 0.75 0.67 0.5
                              ↑ Highest priority
```
...
Bad: Start with "Arazzo multi-step workflows" before "Basic tool descriptions"
