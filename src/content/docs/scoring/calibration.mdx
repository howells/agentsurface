...
Example: In 2025, RFC 9728 protected-resource metadata was optional for Authentication 3/3. The 2026-07-28 MCP spec makes it the discovery mechanism for protected remote servers, so 3/3 now requires it there.
...
Tag the rubric release: `surface-rubric-v1.2` (date: 2026-09-15).
...
ecosystem changes (OAuth 2.1 adoption, the MCP 2026-07-28 spec revision).
```
...
CLI Design is N/A for this project (no CLI ships) and is excluded from the count - N/A agreement isn't evidence of rubric clarity.

**IRR = 5/8 = 0.625** (Poor. MCP Server and Auth criteria need clarification.)
...
**Root cause:** Rubric doesn't say how many tools must carry annotations before the evidence counts.

**Fix:** Update rubric:

> Score 2 requires: annotations on all tools, outputSchema on tools returning structured data.
> Score 3 requires: annotations plus pagination on list operations, RFC 9728 metadata where the HTTP server is protected, and the transport, testing, and consent criteria in the rubric.
...
Solution: Keep the rubric at 10 dimensions. If you want to add one, consider whether it overlaps with existing dimensions or if it is truly foundational.
...
```
SURFACE RUBRIC CALIBRATION — 2026-09-15
...
Inter-rater reliability:
  ...
  By dimension:
    ...
    Discovery: 0.85 (Good)
    Authentication: 0.75 (Good) ← minor ambiguities
