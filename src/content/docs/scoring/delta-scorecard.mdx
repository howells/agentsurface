...
Computed diff between before and after scorecards (typically pre and post transformation) showing per-dimension improvements, overall score change, rating progression (e.g., Agent-tolerant → Agent-ready), and cluster impact identification. Use delta scorecards to track score changes over time and explain which fixes changed the result.
 ...
 */
export function deltaScorecard(before: Scorecard, after: Scorecard): DeltaScorecard {
  // Validate project match
  if (before.project.name !== after.project.name) {
    throw new Error(`Project mismatch: ${before.project.name} vs ${after.project.name}`);
  }
  ...
  const dimensions: DeltaDimension[] = before.dimensions.map((dimBefore) => {
    ...
    const delta = scoreBefore === null || scoreAfter === null ? 0 : scoreAfter - scoreBefore;
  ...
  // Overall scores
  const totalBefore = before.overall.rawScore;
  const totalAfter = after.overall.rawScore;
  const maxBefore = before.overall.maxApplicable;
  const maxAfter = after.overall.maxApplicable;
  const overallDelta = totalAfter - totalBefore;

  // Cluster impact
  const clustersApproached = after.findingClusters
    .filter((cluster) =>
      cluster.findings.some((finding) => finding.currentScore < finding.targetScore),
    )
    .map((c) => c.name);
...
export function formatDeltaScorecard(delta: DeltaScorecard): string {
  ...
  delta.dimensions.forEach((d) => {
    ...
    const deltaStr = d.delta > 0 ? `+${d.delta} ✦` : d.delta < 0 ? `${d.delta} ✗` : "  —";
  ...
  lines.push("  ─────────────────────────────────────────────────────");
  lines.push(
    `  TOTAL: ${delta.totalBefore}/${delta.maxBefore} → ${delta.totalAfter}/${delta.maxAfter} (scaled: ${delta.scaledBefore}/30 → ${delta.scaledAfter}/30)`,
  );
  lines.push(
...
3. **Upgraded authentication** (3 hours)
   - Added OAuth 2.1 Client Credentials grant
   - Scoped tokens (read:api, write:config)
   - JWT validation (iss, aud, exp)
   - Tested token renewal, revocation, and wrong-user/wrong-resource rejection
...
Error Handling:     2/3 (RFC 9457, missing doc_uri)
Tool Design:        2/3 (good descriptions, no toModelOutput)
Context Files:      2/3 (AGENTS.md curated, not yet evolved from real friction)
Testing:            2/3 (tool + error recovery, no evals)
Retrievability:     N/A

Total: 17/24 (scaled: 21/30)
Rating: Agent-ready
```
...
╠════════════════════════════════════════════════════════╣
  ...
  testing                        0/3    2/3   +2 ✦     ↑
  retrievability                  N/A    N/A    —      →

  ─────────────────────────────────────────────────────
  TOTAL: 3/24 → 17/24 (scaled: 4/30 → 21/30)
...
- **Authentication** jumped from 1 → 3: OAuth 2.1 Client Credentials, scoped tokens, and tested renewal/revocation/wrong-resource rejection is complete
...
- **Error Handling** 0 → 2: RFC 9457 with is_retriable (missing doc_uri for 3)
...
- **Context Files** 1 → 2: Curated AGENTS.md (not yet evolved from real friction for 3)
...
- **Score improvement**: +14 points in 13.5 hours of work
...
Tackling these five 2→3 bumps would raise the raw score to 22/24, scaling to roughly 28/30 (Agent-first range).
