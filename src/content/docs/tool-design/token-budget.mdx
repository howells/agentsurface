...
Every tool definition, parameter, and response consumes context tokens. Over many calls in a long session, cost adds up. Minimize tool-definition tokens: descriptions `` `<100` `` tokens first paragraph (link detailed docs for edge cases), parameter descriptions `` `<5` `` tokens, avoid redundancy. Minimize response tokens with concise summaries, pagination (return top-5 not top-1000), and shallow objects. A response 5x larger than necessary is 5x more expensive and may cause context truncation.
...
- **Response design**: concise model-facing summaries
...
For large payloads, return a concise model-facing summary plus structured references. Use framework-specific response-shaping hooks where available, but keep the same design rule: the model sees what it needs to choose the next action, not the full backing payload.
...
When a registry has more tools than a session should see at once, don't inject every definition upfront. MCP itself has no built-in per-tool lazy-loading handler - the server always returns full definitions from `tools/list`. Deferred loading is a client- or provider-side pattern instead: Claude's server-side tool search tool (`tool_search_tool_bm25_20251119` / `tool_search_tool_regex_20251119`, with candidate tools marked `defer_loading: true`) discovers and expands only the tools a turn needs, and the Vercel AI SDK's `activeTools` filters a per-step candidate list client-side. The full mechanics and an embedding-based DIY alternative are in [Tool Curation](/docs/tool-design/tool-curation#dynamic-tool-selection).
...
async function callTool(toolName: string, params: object) {
  ...
  if (cumulativeTokens > maxBudget * 0.9) {
    console.warn(
      `Token budget 90% consumed (${cumulativeTokens} / ${maxBudget}). Consider stopping or switching to cheaper model.`,
    );
  }
...
| Tool count             | `` `~20` `` per agent session  | well past 20 without dynamic selection              |
| Single response        | `` `<1000` `` tokens           | `` `>5000` `` tokens (use pagination or summaries)  |

## See also

- `/docs/api-surface/openapi-for-agents` - pagination patterns in specs
