...
This is model-initiated parallel tool calling - the model issues several tool calls within a single turn - distinct from the concurrent fan-out covered in [Orchestration](/docs/agents/orchestration), where the orchestrator, not the model, decides to run branches concurrently. A tool must be safe to run concurrently with itself and with the other tools in its set before it can be offered for parallel calling; if it is not, say so in its description and serialize it in the host. Provider APIs can switch parallel calls off only for a whole request, not for a single tool, so relying on the model to serialize its own calls is not a control.
