...
`agentify` is the operational part of Agent Surface. It audits a codebase across 11 dimensions, scores each one from `0` to `3`, and produces evidence-backed findings.
...
The skill can stop at a scorecard, produce a transformation plan, or delegate focused improvements to specialist agents.

### The `agents` Skill

`agents` is the scaffolding part of Agent Surface. It helps create and update agent systems: agents, tools, workflows, memory, model routing, browser access, and sandboxed execution.

The skill is project-aware. It recommends Mastra for most TypeScript agent infrastructure, but adapts when a repo already uses the AI SDK, MCP, LangGraph, or Cloudflare Workers.
...
Use `agentify` when you want an evidence-backed audit and a concrete transformation plan.

Use `agents` when you want to create or update agent infrastructure in the shape of the existing project.
...
- [Agents Skill](/docs/agents) for scaffolding and updating agent systems
