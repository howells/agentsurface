...
MCP, Vercel AI SDK, Mastra, LangChain, OpenAI tools, CrewAI, Semantic Kernel, and Amazon Bedrock use different APIs for tool definitions. The shared concepts are the same: a name, a description, an input schema, and a handler. This page covers those portable design rules, then shows how each framework implements them.
