...
Arazzo defines multi-step workflows as executable specifications, preventing agents from inventing their own order. A workflow document declares the sequence of operations, runtime expressions for data flow between steps, and success/failure handlers. This keeps the sequence deterministic and prevents hallucinated steps or omitted operations in complex tasks like flight booking or payment processing.

- Latest published specification: Arazzo 1.1.0.
- Arazzo references one or many OpenAPI (and, since 1.1.0, AsyncAPI) specs and maps them into workflows
- Each step binds to an operationId and passes data between steps via runtime expressions
- 1.1.0 adds a Selector Object for extracting a value from a JSON or XML response with JSONPath, XPath, or JSON Pointer
- Control flow primitives: retry, goto recovery step, end
...
- Success and failure actions (retry, goto a recovery step, end)
- Reusable parameter and input components

## Core Structure

An Arazzo document has five top-level fields: `arazzo`, `info`, `sourceDescriptions`, `workflows`, and `components`.
...
Each `sourceDescriptions` entry's `type` is `openapi`, `asyncapi`, or `arazzo` (a nested reference to another Arazzo document). The `asyncapi` type, added in 1.1.0, lets a workflow step bind to an AsyncAPI operation - orchestrating a publish/subscribe or messaging step alongside REST steps in the same workflow, using the same `$sourceDescriptions.<name>#<operationId>` reference syntax.
...
### Selector Object (Arazzo 1.1.0)

For extraction that a plain runtime expression can't reach - a JSONPath query, an XPath into an XML body - 1.1.0 adds a Selector Object with three required fields: `context` (the runtime expression identifying the data source, e.g. `$response.body`), `selector` (the query itself), and `type` (`jsonpath`, `xpath`, or `jsonpointer`):
...
Use a Selector Object where the plain `$response.body#/path` shorthand can't express the query - array filtering, XML namespaces, or a JSON Pointer built from a runtime value.

Criterion objects in `successCriteria` use the same `context` and `type` fields but carry the expression in `condition`, not `selector`.
...
Validate Arazzo documents with the `@jentic/arazzo-validator` package before generating tools:

```bash
npx @jentic/arazzo-validator validate booking-workflows.arazzo.yaml
```
