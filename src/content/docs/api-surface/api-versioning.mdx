...
When a field is renamed, a developer sees the migration guide and updates their code. An agent's tool definition must keep working until someone on the team explicitly re-tools it.
...
| Fixing a description that was factually wrong | Sometimes | Monitor CUJ pass rates before and after          |

<Callout type="warn">
  Changing an `operationId` is the most impactful breaking change. All generated SDKs, MCP servers,
  and tool definitions use the `operationId` as the function name. Treat `operationId` as immutable
...
**Sunset and Deprecation headers:** Include both HTTP headers in responses from deprecated endpoints so agent frameworks can surface warnings automatically, instead of relying on an agent to have read a changelog:

```http
HTTP/1.1 200 OK
Deprecation: @1710460800
Sunset: Thu, 01 Jan 2026 00:00:00 GMT
Link: <https://docs.example.com/migration/v2-v3>; rel="deprecation"
Link: <https://docs.example.com/migration/v2-v3>; rel="sunset"
```

`Sunset` ([RFC 8594](https://www.rfc-editor.org/rfc/rfc8594.html), Informational) carries an HTTP-date: the point after which the resource is expected to stop responding. Treat it as a hint, not a guarantee - the RFC itself says so.

`Deprecation` ([RFC 9745](https://www.rfc-editor.org/rfc/rfc9745.html), Standards Track) signals that a resource is already, or will be, deprecated. Its value is a Structured Field Item that MUST be a Date, written as `@` followed by a Unix timestamp - not an HTTP-date string. `Deprecation: @1710460800` means the resource was deprecated at that timestamp; deprecation does not itself change the resource's behaviour, it only announces the lifecycle stage. Pair it with `Sunset` to say when the endpoint actually stops working.

Both headers can carry a companion `Link` header - `rel="deprecation"` or `rel="sunset"` - pointing at migration documentation, for either header independently.
...
- [ ] Deprecated endpoints include `Sunset` (RFC 8594, HTTP-date) and `Deprecation` (RFC 9745, `@`-prefixed Unix timestamp) HTTP response headers
...
- [RFC 8594: The Sunset HTTP Header Field](https://www.rfc-editor.org/rfc/rfc8594.html) - informational, HTTP-date retirement signal
- [RFC 9745: The Deprecation HTTP Header Field](https://www.rfc-editor.org/rfc/rfc9745.html) - standards track, `@`-timestamp deprecation signal
