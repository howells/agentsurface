...
<Callout type="warn">
  Long descriptions hurt agent performance more than they help. A 2000-token description does not
  make an agent smarter about when to use a tool — it makes every other tool harder to select.
  Target under 150 words per operation description.
</Callout>
...
- Minimize the steps that need a person: self-serve API keys, a sandbox or test-mode key that works without a sales call, and no mandatory approval gate before a first successful call.
