...
OpenAPI extensions like x-action and x-category add semantic metadata to operations. The AgenticAPI ACTION taxonomy classifies operations into six categories: Acquire (read), Compute (transform), Transact (create/modify), Integrate (sync), Orchestrate (multi-step), and Notify (communicate). This enables agents to make safer routing decisions.
...
paths:
  ...
  /reports/generate:
    post:
      ...
      x-action: generate_sales_report
      x-category: Compute
...
Speakeasy provides `x-speakeasy-mcp` for customizing how MCP tool generation treats individual operations.
...
## Discovery Endpoint: `GET /actions`

Publishing a discovery endpoint lets agents find your action catalog without reading the full OpenAPI spec. There's no standardized HTTP method or path for this - OpenAPI 3.2 added a new `QUERY` method, but nothing called `DISCOVER`. A plain `GET` at a well-known path works and needs no special client support:
...
[LAPIS](https://github.com/cr0hn/LAPIS) (Lightweight API Specification for Intelligent Systems) is a compact representation format for OpenAPI specs, designed for LLM context windows. Benchmarked against five production API specs (including GitHub and Twilio), it averages an 85.5% token reduction compared to standard OpenAPI YAML while retaining the information agents need to select and call operations.

A full OpenAPI spec in YAML might be 50,000 tokens. The same spec in LAPIS format is typically well under 10,000 tokens. This matters when you are stuffing tool definitions into a model context window.
...
The extensions above - especially `x-speakeasy-mcp` - feed directly into tools that generate MCP servers from your OpenAPI spec. Generation is only a starting point: a naive 1:1 mapping produces 50–300 tools that overwhelm agents, with descriptions written for human developers rather than agent reasoning. The correct workflow is generate → prune → rewrite → add composites, targeting 10–20 tools per domain.
...
- [ ] Operations are categorized with `x-category` using the ACTION taxonomy
