...
Detect regressions before production via PR-gated evals. Use layered strategy: Stage 1 (`<1` min, deterministic routing tests, temp 0, mocked tools, Vitest); Stage 2 (1-3 min, real calls, real LLM, baseline comparison vs. main); Stage 3 (optional, 5-10 min nightly, red-team suite). Track statistical significance (gate on pass@k improvements across runs rather than any single run), token budget guardrails (cost per eval case), and baseline regression detection (compare to main-branch results). Fail fast strategy: fail on Stage 1, run Stage 2 only if Stage 1 passes, skip Stage 3 on PRs.
...
on:
  pull_request:
    branches: [main]
  schedule:
    - cron: "0 3 * * *" # nightly, drives the redteam job below
      ...
      - name: Comment results on PR
        ...
        with:
          script: |
            ...
            const baselinePass = (baseline.summary.pass_rate * 100).toFixed(1);
            const currentPass = (current.summary.pass_rate * 100).toFixed(1);
