...
Agent testing measures statistical performance under non-determinism. Covers the full lifecycle: evaluation frameworks and metrics (pass@k, tool-routing accuracy), LLM-as-judge calibration, specific platforms (Braintrust, Promptfoo, Vitest), red-teaming for injection/jailbreak, observability, and CI/CD integration. Start with framework definition, then pick a platform matching your workflow.
...
Traditional testing verifies deterministic behavior - given input X, does output Y occur every time? An agent may take different paths, call tools in different orders, or succeed on the fifth attempt when it failed on the first. A single success or failure tells you nothing.
...
- **Evaluation framework** - `/docs/testing/evaluation-framework` - dataset, task, scorer, aggregation loop
- **Braintrust** - `/docs/testing/braintrust` - end-to-end eval platform with dataset versioning and trace integration
- **Promptfoo** - `/docs/testing/promptfoo` - YAML-first evals with strong red-teaming, acquisition announced by OpenAI, close unconfirmed
...
Evals answer whether a change passed against a dataset. A/B testing answers whether it performed better in front of real users, and complements evals: evals gate a merge cheaply before a change ever reaches traffic; A/B testing measures the thing evals can only approximate. It needs enough live traffic to reach statistical significance, so it becomes practical once volume is high enough to split without starving either arm - before that point, lean on [offline and online evals](/docs/testing/evaluation-framework#offline-vs-online-evals) instead. Online evals score output quality on sampled traffic; A/B testing compares variants on the product outcome.
