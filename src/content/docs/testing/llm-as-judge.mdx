...
LLM-as-judge (also called model-based grading) evaluates outputs that have no deterministic ground truth - plan quality, parameter semantics, summary coherence. A judge model reads the task, agent output, and reference (if available), then scores it. LLM judges are biased and systematically prefer certain outputs even when they are not actually better.
...
A judge model tends to prefer outputs from itself or similar models. If you use GPT-5.6 to judge outputs from Claude Opus 5.5, the judge may systematically overrate outputs that look like GPT-style reasoning.
...
Score this output on a scale of 0-1. Return JSON: {"score": number, "reasoning": string}
  ...
  // Low stdDev = judges agree = confident score; high stdDev = flag for human review
...
## Prefer Binary or Categorical Labels

A judge asked to rate on a 1-10 scale produces noisy, poorly calibrated numbers - the gap between a 6 and a 7 is rarely meaningful, and different judges (or the same judge on different days) drift across the scale inconsistently. A judge asked for a binary label (pass/fail) or a small set of named categories (correct / partially correct / incorrect) produces a more reproducible signal, because there is less room for the judge to invent its own scale. Reserve a numeric score for cases where the granularity genuinely matters and has been calibrated against human labels - see anchor datasets, above - rather than as the default output shape.

## Cost optimization

LLM judges are expensive at scale. Strategies to reduce cost:

- **Use cheaper judges for pre-screening.** Haiku 4.5 for initial triage, then Opus 5.5 for borderline cases.
