...
The metrics above are computed from a run; a judge model is asked about one. Below both sits a third tier that is easy to overlook because it looks too simple to name: **micro-scorers** that answer a single narrow question about a run, deterministically, in microseconds, for zero tokens.
...
Individually these are assertions you could write by hand in any test file. Naming them as a tier buys two things.
