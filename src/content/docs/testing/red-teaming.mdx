...
Red-teaming evaluates agent resilience against malicious inputs. Attacks range from prompt injection to data exfiltration loops to tool abuse. Red-team suites measure failure modes that happy-path evals never exercise, so they need explicit test cases. This is the offense side; [Guardrails](/docs/agents/guardrails) is the defense side that stands in the production request path.
...
jobs:
  redteam:
    ...
    steps:
      ...
      - name: Check all attacks were resisted
        run: |
...
- OWASP LLM Top 10: https://genai.owasp.org/ (the 2025 edition, published November 2024)
...
- Promptfoo red-teaming docs: https://www.promptfoo.dev/docs/red-team/quickstart/
