...
- Server caches both successful and failed responses, so a permanent error doesn't get retried into a different outcome
...
The `Idempotency-Key` header comes from an IETF draft (`draft-ietf-httpapi-idempotency-key-header`) that expired April 2026 without advancing to a standard; it is not an active working-group document. Treat the header as a de facto convention established by wide production use - Stripe's implementation is the most-cited reference - rather than a ratified spec. Generate a unique key per logical operation and send it on every retry:
...
Idempotency keys are required for operations that:
