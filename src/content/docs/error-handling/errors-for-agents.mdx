...
- Never return stack traces or internal exception details; sanitize before responding

When an agent meets a vague error, its most common response is to silently abandon the task. A human encountering a vague "something went wrong" error will read it, shrug, and try something different. An agent encountering the same message has no information to act on.

Agent-recoverable errors are designed differently. They contain the structured information the agent needs to decide: should I retry this request unchanged? Should I modify the request and try again? Should I stop and escalate? The goal is to make as many failures recoverable as possible: a precise error message tells the agent exactly what to do next.
...
// Pattern 1: Retry unchanged - transient failure
{
  ...
  "suggestions": ["Retry the same request after the retry_after_ms delay"]
}
...
// Pattern 2: Modify and retry - validation failure
{
  ...
  "invalid_fields": [
    { "field": "/due_date", "reason": "Must be a future date", "received": "2025-01-01" }
  ]
}
...
```json
{
  ...
  "invalid_fields": [
    ...
    },
    {
      ...
      "received": "2025-01-01",
      "expected": "ISO 8601 date string for a date after today (2026-09-24)"
    }
  ],
  "suggestions": [
    "Add an 'amount' field with a positive integer (e.g., 5000 for $50.00)",
    "Change 'currency' from 'dollars' to 'USD'",
    "Change 'due_date' to a date after 2026-09-24"
  ]
}
```
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      ...
      if (!isProblemDetails(error)) {
        throw error; // Not a structured error - nothing to act on
      }

      // Recoverable means "safe to retry unchanged" OR "we have a correction
      // hint" - is_retriable alone would miss Pattern 3 (next_operation with
      // is_retriable: false)
      const canRecover = error.is_retriable || error.next_operation || error.invalid_fields;
      if (!canRecover) {
        throw error; // No retry signal and no correction hint - stop immediately
      }
...
## Agent-Friendly 404s and 410s

A missing or removed resource should return the real status - 404 for not found, 410 for gone - never a 200 with an empty or generic page. A "soft 404" gives an agent no signal to stop treating the response as real content, and it will often extract an answer from whatever boilerplate came back.

The status code alone is only half the signal. The body should say what happened and where to go next, in a format the agent can actually read: Markdown, or [RFC 9457](/docs/error-handling/rfc-9457) `application/problem+json` when the request's `Accept` header asks for structured data.
...
// 404, application/problem+json
{
  ...
  "status": 404,
  "detail": "No invoice with id 'inv_0000' exists. Invoice IDs look like 'inv_01HV3K8MNP2QRS3TUVWX'.",
  "is_retriable": false,
  "suggestions": ["List existing invoices with GET /invoices to find a valid id"]
}
```
...
No invoice with id `inv_0000` exists. Invoice IDs look like `inv_01HV3K8MNP2QRS3TUVWX`.

- List existing invoices: `GET /invoices`
- Documentation index: [docs.example.com](https://docs.example.com)
```

Ora's `agent-friendly-404` check scores this in two tiers: "A real 404/410 status earns partial credit. Full credit also requires a Markdown error body, so agents can read what happened and where to go next" ([ora.ai/api/checks](https://ora.ai/api/checks)).

## Checklist

- [ ] Every error body includes `is_retriable` - agents should never have to guess
- [ ] Missing or removed resources return a real 404/410 status, never a 200 soft-404
