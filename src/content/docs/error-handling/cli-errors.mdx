...
CLI errors must be machine-readable. Use exit codes (0 success, 1 failure, 2 usage, 3 not found, 4 permission denied, 5 conflict) to categorize failures. In non-TTY mode, emit structured JSON on stderr with stable snake_case error codes, suggestions, and failing_input. Never emit stack traces.
...
- Exit codes: 0 (success), 1 (failure), 2 (usage), 3 (not found), 4 (permission denied), 5 (conflict)
...
| 1    | Failure           | An unexpected error - safe to retry with exponential backoff                          |
...
| 4    | Permission denied | Authentication or authorization failed - do not retry until credentials are refreshed |
...
case $EXIT in
  ...
  4) echo "Permission denied - refresh credentials" ;;
...
When the agent provides invalid arguments, report per-argument failures in a structure that mirrors the HTTP validation format, with `invalid_args` and `arg` in place of `invalid_fields` and `field`:
...
```json
{
  ...
  "invalid_args": [
    ...
    },
    {
      ...
      "received": "2025-01-01",
      "expected": "A date after 2026-09-24"
    }
  ],
  "suggestions": [
    "Change --amount to a positive integer (e.g., --amount 5000)",
    "Change --due-date to a future date (e.g., --due-date 2026-10-01)"
  ]
}
```

This format mirrors the HTTP validation error structure closely enough that the same agent parsing logic can walk both, once it accounts for `invalid_args`/`arg` in place of `invalid_fields`/`field`.
...
{"id": "inv_01HV3K8MNP", "status": "sent", "sent_at": "2026-09-25T10:22:01Z"}
# exit: 0
...
- [ ] Exit codes follow the five-level scale (0 success, 1 failure, 2 usage, 3 not found, 4 permission denied, 5 conflict)
