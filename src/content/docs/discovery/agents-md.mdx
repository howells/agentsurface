...
AGENTS.md is a tool-neutral repository context file for coding agents. It answers key questions agents have before writing code: how to build, test, lint, and run the project. The file is discovered by walking up the directory tree and supports precedence (closer files override farther ones). Maximum size: 32 KiB.
...
`AGENTS.md` is a tool-neutral convention for repository context files. Unlike vendor-specific formats, it gives any agent that reads project files a predictable place to find commands, boundaries, and working notes.
...
Return RFC 9457 Problem Details for all errors.
```
