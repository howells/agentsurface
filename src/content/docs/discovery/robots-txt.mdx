...
Cloudflare introduced Content Signals, a structured way to declare AI usage permissions that sits alongside `robots.txt`. It is configured at the Cloudflare dashboard level and injects a policy header into responses:

```
Content-Signals-Policy: search=yes, ai-input=yes, ai-train=no
```
...
This header approach has an advantage over `robots.txt`: it applies per-response rather than per-path, which means you can vary the policy based on the request (authenticated users versus public content, for example).
