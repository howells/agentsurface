...
/.well-known/ is an IETF standard (RFC 5785) for site-wide metadata. Agents use it for capability discovery without prior knowledge. Key files include /.well-known/ai (AI capabilities), /.well-known/mcp (MCP server locations), and /.well-known/openapi.json (API specs). Publishing these enables agents to discover and use your system programmatically.

- /.well-known/ai: JSON describing AI-facing capabilities (64 KB max)
- /.well-known/mcp: MCP server endpoints and transports
- /.well-known/openapi.json: Link to OpenAPI specification
- /.well-known/oauth2-authorization-server: OAuth metadata
...
MCP server discovery is described in a Server-Endpoint Proposal (SEP) in progress. The proposed path is `/.well-known/mcp.json`:
...
This format is still evolving. Check the MCP specification repository for the latest field definitions before implementing.
