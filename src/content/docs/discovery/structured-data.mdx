...
- FAQPage and HowTo schema are still valid Schema.org markup for agents and other parsers, but Google no longer shows either as a Search rich result (HowTo since September 2023, FAQPage since May 2026)
...
- Schema must match visible content exactly; mismatched schema is a trust signal search engines penalize
...
## Page identity and presentation metadata

Set a descriptive page title, the document language (`html lang`), and a canonical URL. Use accurate Open Graph type and image metadata when consumers present shared previews. Keep alternate Markdown and regional versions tied to their actual source. These signals complement structured data; they do not establish ranking or factual accuracy. Verify the rendered head and the fetched document rather than relying on framework configuration alone.
...
JSON-LD must exactly match the visible content on the page. If your FAQ schema contains a question-and-answer pair that does not appear in the page body, search engines will penalize the page and agents may distrust all structured data from your domain. The schema is a machine-readable version of the page.

## FAQPage

Use `FAQPage` for any content structured as questions and answers: FAQ sections, definition pages, policy explanations, troubleshooting guides. Google removed FAQ rich results from Search in May 2026, so this schema no longer earns Google's rich-result presentation. Keep it anyway: it is still valid Schema.org markup, and agents and other parsers that read JSON-LD directly still benefit from question-and-answer content marked up explicitly rather than left to prose inference.
...
Use `HowTo` for procedural content: setup guides, integration walkthroughs, step-by-step tutorials. Google removed `HowTo` rich results from Search in September 2023; do not expect a Google rich result from this schema. Keep it for agents and other parsers that read structured step data directly.
...
- **Google Rich Results Test** - `search.google.com/test/rich-results` - validates whether Google can process your schema and whether it qualifies for rich results. It no longer covers `FAQPage` (Google dropped the rich result itself on 7 May 2026, with Rich Results Test support following in June 2026) or `HowTo` (removed September 2023); use it for the schema types Google still surfaces, and the Schema.org Validator for FAQPage and HowTo.
...
Use sameAs for verified equivalent identities, not arbitrary mentions. Wikipedia/Wikidata eligibility, official-site linking, and truthful Organization details are covered in [Commercial and Entity Discovery](/docs/discovery/commercial-and-entity-discovery).
