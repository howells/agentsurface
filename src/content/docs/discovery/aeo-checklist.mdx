...
This tiered checklist organizes AEO implementation from essential foundation items (Tier 1) through comprehensive support (Tier 4). Tier 1 includes robots.txt, llms.txt, and OpenGraph tags — low effort, high impact. Later tiers add JSON-LD, content negotiation, OpenAPI links, and well-known endpoints. Each item links to detailed implementation guidance.
...
- Tier 4: Well-known endpoints, advanced discovery, monitoring
...
- [ ] `mcp.json` published if you run an MCP server (draft standard — check current spec)
...
- [ ] Authentication works with OAuth 2.0 Device Authorization or Client Credentials
...
- [ ] `Content-Signals-Policy: search=yes, ai-input=yes, ai-train=no` header deployed
