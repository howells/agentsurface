...
<Callout type="info">
  ...
  read them once and cache them, so if you change an annotation on a running server, connected
  clients will not see the change until they re-fetch the tool list. Under `2026-07-28` there is no
  session to cache against: sessions are removed, `tools/list` results no longer vary per
  connection, and cache lifetime becomes explicit through the required `ttlMs` and `cacheScope`
  fields on list results (SEP-2549). Set a `ttlMs` you can live with, because it is the window in
  which a stale annotation stays in circulation.
</Callout>

## Related

- [Real-World MCP Patterns](./real-world-examples) - GitHub's `--read-only` flag gating registration on `readOnlyHint`, an applied use of these annotations
...
- [MCP Specification 2026-07-28](https://modelcontextprotocol.io/specification/2026-07-28/) - current tool annotation definitions
