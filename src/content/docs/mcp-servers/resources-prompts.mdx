...
export function registerInvoiceResource(server: McpServer, config: ServerConfig): void {
  server.resource(
    "invoice",
...
<Callout type="info">
  Subscriptions are only meaningful when your server has a persistent connection to the client, such
  as with Streamable HTTP or SSE transport. A stdio server that stays resident for the client
  session can maintain subscription state; one invoked fresh per call, as some CLI-launched servers
  are, cannot. See [Transport Selection](./transports) for details.
</Callout>

<Callout type="warn">
  Current `2026-07-28` behavior (SEP-2575): `resources/subscribe` and `resources/unsubscribe` are
  removed, along with the HTTP GET endpoint that carried server-to-client notifications. Both are
  replaced by a single `subscriptions/listen` request that opens one long-lived POST-response
  stream, with the client opting in per notification type - `resourcesListChanged` and
  `resourceSubscriptions` are the two that apply here - and each notification tagged with
  `io.modelcontextprotocol/subscriptionId`. The notifications you emit keep the same method names
  and payloads, so the server-side change is where the stream is established rather than what
  travels on it. List and read results also gain required `ttlMs` and `cacheScope` fields
  (SEP-2549), which give clients a defensible reason to poll less. The code above is correct for
  `2025-11-25` and today's SDKs.
</Callout>
...
export function registerBillingAssistant(server: McpServer): void {
  server.prompt(
    ...
    async (args) => {
      const scope = args.customer_id ? `for customer ${args.customer_id}` : "across all customers";
...
<Callout type="warn">
  Do not use resources as a workaround for tools with large responses. If an agent needs to search
  for data dynamically, that is a tool. Resources are for application-controlled, URI-addressable
  data - not for query results that depend on runtime parameters beyond what a URI template can
  express.
</Callout>
