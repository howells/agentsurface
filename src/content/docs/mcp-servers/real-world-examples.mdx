...
- Logging: write to stderr on stdio, OpenTelemetry on HTTP - the MCP logging facility is Deprecated in `2026-07-28`
- GitHub, Stripe, Vercel all use tens of tools per server, not hundreds
...
Stripe handles all deployment, scaling, versioning, and API compatibility. The OAuth flow delegates access - users authorize specific Stripe capabilities without sharing their API keys. This is the pattern the `2025-11-25` MCP spec mandates for public servers.
...
Vercel's MCP server is built on `mcp-on-vercel`, a template that demonstrates the full production deployment pattern. The server exposes Vercel API operations and is used as a reference implementation for the `mcp-handler` package.
