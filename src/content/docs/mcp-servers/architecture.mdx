...
description: Structuring MCP servers for testability, maintainability, production reliability, and public distribution
...
This has practical consequences for agents:
...
Separate server construction from server startup. The `createServer()` factory creates and configures the server but does not connect it to any transport. The `cli.ts` entry point calls `createServer()` and then connects it to a transport.
...
const CreateInvoiceSchema = z.object({
  customer_id: z.string().uuid().describe("The UUID of the customer to invoice"),
  amount_cents: z
    .number()
    .int()
    .positive()
    .describe("Invoice amount in cents to avoid floating point issues"),
  currency: z.enum(["usd", "eur", "gbp"]).describe("ISO 4217 currency code"),
  due_date: z
    .string()
    .regex(/^\d{4}-\d{2}-\d{2}$/)
...
<Callout type="warn">
  Never declare `experimental_sampling` in capabilities unless you have implemented the full
  sampling request handler. Clients that send sampling requests to an unimplemented handler will
  receive protocol errors that are difficult to debug.
</Callout>

<Callout type="info">
  Current `2026-07-28` behavior: capabilities are no longer negotiated once at connect time. The
  `initialize` handshake is removed, clients send their capabilities in `_meta` on every request,
  and servers may advertise theirs through an optional `server/discover` method. The `logging`
  capability is Deprecated (SEP-2577), and `resources.subscribe` gives way to opting into
  `resourceSubscriptions` on a `subscriptions/listen` stream. Capability objects also gain an
  `extensions` field carrying reverse-DNS identifiers such as `io.modelcontextprotocol/tasks`. The
  declaration above is a legacy `2025-11-25` compatibility example; the discipline of declaring only
  what you implement carries over unchanged.
</Callout>
...
function loadConfig(): ServerConfig {
  ...
  for (const key of required) {
    if (!process.env[key]) {
      throw new Error(
        `Missing required environment variable: ${key}. ` + `See README for setup instructions.`,
      );
    }
  }
...
## Publishing a Server Publicly

A server built to power your own in-app agent can also become a distribution channel: the same tool registrations, served over Streamable HTTP on a stable public URL, let external clients (Claude, ChatGPT, an IDE) use your product's capabilities directly. The architecture does not change - you still build with the `createServer()` factory and register tools the same way - but three things become mandatory once the server is public rather than internal:

1. **Streamable HTTP, not stdio.** External clients cannot launch your process as a subprocess. See [Transport Selection](./transports) for the stateless HTTP setup.
2. **OAuth 2.1, not a shared bearer token.** A public server cannot distribute a static secret to every user. See [MCP Authentication](./authentication) for the full server-implementer flow, including the resource-indicator and protected-resource-metadata requirements RFC 8707 and RFC 9728 impose.
3. **Discovery metadata clients can find.** Publish `/.well-known/oauth-protected-resource` (RFC 9728) so clients can locate your authorization server - this is the MUST-level discovery mechanism in the current spec. An informal `/.well-known/mcp/server-card.json` document is under active proposal (SEP-2127, Server Card Working Group) but is still Experimental and not part of the ratified spec; do not build a public launch around it yet.

Register only the tools a given caller is authorized to use - Stripe's `configuration.actions` pattern in [Real-World MCP Patterns](./real-world-examples) is the reference example, and the same scope-gated registration applies whether the caller is your own frontend or an external MCP client.

To be discoverable beyond your own docs, publish an entry to the official [MCP Registry](https://registry.modelcontextprotocol.io) (namespace verified via GitHub OAuth or a DNS TXT record) and document connection setup for the clients you expect - a `claude_desktop_config.json` snippet for stdio-style local config, and your server's public URL for clients that add remote MCP servers directly. The registry added Cargo (crates.io) as a package registry type in [v1.8.0](https://github.com/modelcontextprotocol/registry/releases) (2026-07-13), alongside npm, PyPI, NuGet, and OCI; v1.8.1 (2026-08-06) closed a namespace-takeover hole by rejecting `github.io` domains during DNS/HTTP token-exchange verification. The registry itself is still a preview release - expect breaking changes and possible data resets before a stable release. Track external usage (rate limits, per-caller quotas) the same way you would any public API; nothing about that is MCP-specific.

## Related

- [Transport Selection](./transports) - stdio vs. Streamable HTTP and stateless mode
- [MCP Authentication](./authentication) - OAuth 2.1, PKCE, and token validation for public servers
- [Real-World MCP Patterns](./real-world-examples) - scope-gated tool registration and hosted-server patterns from GitHub, Stripe, and Vercel
