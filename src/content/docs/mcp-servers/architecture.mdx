...
<Callout type="info">
  Changing in `2026-07-28`: capabilities are no longer negotiated once at connect time. The
  `initialize` handshake is removed, clients send their capabilities in `_meta` on every request,
  and servers advertise theirs through a mandatory `server/discover` method. The `logging`
  ...
  declaration above is correct for `2025-11-25` and today's SDKs; the discipline of declaring only
