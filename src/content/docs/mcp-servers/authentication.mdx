...
MCP servers require authentication matching their scope. Bearer tokens suffice for small, controlled environments. OAuth 2.1 is mandated for public servers, a requirement in place since the `2025-03-26` revision introduced MCP's authorization framework. Three approaches by complexity: bearer tokens (simple, environment-controlled), identity proxies (add authorization layer), and OAuth 2.1 (standards-based, public-scale). Even internal servers need basic defense.
...
The current `2026-07-28` MCP specification requires OAuth 2.1 for publicly accessible protected servers and hardens the predecessor model. Servers used only within a controlled environment (single user, stdio, internal network) can use simpler approaches. This page covers all three patterns in order of complexity.
...
<Callout type="info">
  STDIO transport **SHOULD NOT** follow the OAuth authorization flow at all - the MCP `2026-07-28`
  specification says implementations using STDIO should instead retrieve credentials from the
  environment, which is exactly the bearer-token-via-env-var pattern above. Reach for OAuth 2.1 only
  once a server is reachable over an HTTP-based transport. Source:
...
OAuth 2.1 with PKCE is mandatory for publicly accessible MCP servers, a requirement in place since the `2025-03-26` revision introduced MCP's authorization framework. It is the only authentication method that supports delegated authorization - where an agent acts on behalf of a user without the user handing over their credentials.
...
<Callout type="info">
  Current `2026-07-28` behavior: clients must validate the authorization response's `iss` parameter
  per RFC 9207 (SEP-2468) and must key stored client credentials by issuer, using them only with the
  authorization server that issued them and re-registering when an issuer changes (SEP-2352). RFC
  7591 dynamic client registration is Deprecated in favor of Client ID Metadata Documents, and where
  you still accept DCR the `application_type` field becomes required (SEP-837). The server-side work
  on this page is unaffected - you validate a bearer token per request either way, which is exactly
  what the stateless core assumes now that sessions are gone. See [MCP
  Authorization](/docs/authentication/mcp-auth-model) for the full list.
</Callout>

<Callout type="warn">
  MCP clients **MUST** implement Resource Indicators for OAuth 2.0 (RFC 8707): the `resource`
  parameter identifying your server's canonical URI (for example `https://billing-mcp.example.com`)
  is required in **both** the authorization request and the token request, not only reflected back
  in the `aud` claim you validate below. A client that omits it is spec-non-conformant; a server
  cannot add the parameter on the client's behalf, but audience validation on the token you receive
  is how you catch a client that skipped it. Source:
  https://modelcontextprotocol.io/specification/2026-07-28/basic/authorization
</Callout>
...
authUrl.searchParams.set("resource", "https://billing-mcp.example.com"); // RFC 8707 - required
```

The client repeats the same `resource` parameter in the token request that exchanges the code for an access token. Omitting it on either request is a protocol violation, not an optional hardening step.
...
The `mcp-handler` package provides a `withMcpAuth` wrapper that integrates OAuth validation directly into the Vercel route handler:
...
export const { GET, POST, DELETE } = withMcpAuth(mcpHandler, validateAuth);
...
**5. Storing tokens in plaintext.** Access tokens and refresh tokens stored in plaintext in a database or log file are readable by anyone with database or log access. Encrypt tokens at rest. Never log token values, even in debug output.
