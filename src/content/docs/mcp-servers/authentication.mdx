...
MCP servers require authentication matching their scope. Bearer tokens suffice for small, controlled environments. OAuth 2.1 is mandated for public servers, from the `2025-11-25` revision onward. Three approaches by complexity: bearer tokens (simple, environment-controlled), identity proxies (add authorization layer), and OAuth 2.1 (standards-based, public-scale). Even internal servers need basic defense.
...
The `2025-11-25` MCP specification mandates OAuth 2.1 for publicly accessible servers, and the newer `2026-07-28` revision keeps that requirement while hardening it. Servers used only within a controlled environment (single user, stdio, internal network) can use simpler approaches. This page covers all three patterns in order of complexity.
...
OAuth 2.1 with PKCE is mandatory for publicly accessible MCP servers as of the `2025-11-25` MCP specification. It is the only authentication method that supports delegated authorization — where an agent acts on behalf of a user without the user handing over their credentials.
...
<Callout type="info">
  Changing in `2026-07-28`: clients must validate the authorization response's `iss` parameter per
  RFC 9207 (SEP-2468) and must key stored client credentials by issuer, using them only with the
