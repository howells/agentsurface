...
This cuts the tool count to two per resource and matches how agents use APIs: they either know what they want (get by ID) or they need to find it (search with filters).
...
The `x-speakeasy-mcp` extension works with Speakeasy's generator and can be added to any OpenAPI spec as a vendor extension. This section covers how it drives the generation workflow; for the full extension surface - tool-name overrides, scope grouping, parameter-description overrides, and pattern-based scope mapping - see [OpenAPI Extensions](/docs/api-surface/openapi-extensions). You can annotate your spec incrementally, adding MCP configuration to the operations you want to expose without modifying the spec's existing structure:
...
<Callout type="warn">
  ...
  works but that agents use poorly - wrong tool selection, missing context, redundant calls. Most of
  the final quality comes from the rewriting phase.
</Callout>
...
- [OpenAPI Extensions](/docs/api-surface/openapi-extensions) - the `x-speakeasy-mcp` and `x-action` extension mapping these generators consume
