...
<Callout type="info">
  Current `2026-07-28` behavior: the `initialize` handshake this page's `client.connect()` calls
  perform is removed. Capabilities move from a one-time connect-time negotiation to `_meta` on every
  request, so a Level 2 test asserting on the connect response is asserting on a step the wire
  protocol no longer has. The code below is the `2025-11-25` shape, which the SDK still supports;
  see [Server Architecture](./architecture) for what changes under `2026-07-28` and update these
  tests when your SDK version does.
</Callout>
...
async function callTool(server: McpServer, toolName: string, params: Record<string, unknown>) {
...
By default, the Inspector's web UI opens on port 6274. If that port is in use, set `CLIENT_PORT`:

```bash
CLIENT_PORT=6275 npx @modelcontextprotocol/inspector node dist/cli.js
```
...
- Inspect capabilities carried in `_meta` on requests and, where the server implements it, the optional `server/discover` response
...
<Callout type="info">
  The Inspector connects using the same MCP client library used by Claude Desktop and other MCP
  clients. If something works in the Inspector but fails in your actual agent client, the issue is
  in the client configuration or authentication layer, not the server implementation.
</Callout>
