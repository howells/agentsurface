...
const delegatedToken = await delegateToSubAgent(
  orchestratorToken,
  subAgentSelfSignedToken, // a security token identifying "sub-agent-summarize", not a bare ID
...
Use Token Exchange when the agent already has a valid subject token or delegation to narrow. If the agent first needs to register or claim a user-bound credential, use [auth.md](/docs/authentication/auth-md) or another explicit registration flow before token exchange.
...
const delegatedToken = await exchangeForDelegatedAccess(
  userJwt,
  agentSelfSignedToken, // a security token identifying "agent-process-invoices", not a bare ID
  "https://invoices-service.example.com/",
  ["invoices:read"],
);
```
...
- [auth.md](/docs/authentication/auth-md) - registering or claiming user-bound credentials before delegation
