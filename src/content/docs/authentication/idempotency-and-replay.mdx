...
async function handlePayment(req: Request): Promise<Response> {
  ...
  await redisClient.set(cacheKey, JSON.stringify(result), { EX: IDEMPOTENCY_TTL });

  return Response.json(result, { status: 200 });
}

async function processPayment(data: { amount: number; currency: string; description: string }) {
