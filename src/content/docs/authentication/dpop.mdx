...
MCP has encouraged DPoP for remote servers that expose sensitive operations since the `2025-11-25` revision, and the current `2026-07-28` revision retains that guidance. An MCP server can require DPoP proofs on every resource access:
...
<Callout type="info">
  Two `2026-07-28` changes make DPoP easier to reason about on MCP servers. Sessions are removed, so
  there is no connect-time authentication step whose proof a later request could ride on — every
  request carries its own token and its own proof, which is what DPoP wanted anyway. And client
  credentials must now be keyed by issuing authorization server (SEP-2352), which means the
  `dpop_jkt` thumbprint you validate is unambiguously tied to one issuer's key material rather than
