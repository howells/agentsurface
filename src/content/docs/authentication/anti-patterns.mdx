...
Nine patterns that break agent auth silently: an authorization flow that does not fit the acting identity, CAPTCHA on token endpoint, session cookies, long-lived broad-scope keys, overly broad scopes, hardcoded secrets, per-request token calls, tokens without audience restriction, and tokens without DPoP. All fail silently or fail late.

| Pattern                             | Why it breaks                                  | Fix                                                                                     |
| ----------------------------------- | ---------------------------------------------- | --------------------------------------------------------------------------------------- |
| Wrong grant for the acting identity | User consent or service access cannot complete | Authorization Code + PKCE for user delegation; Client Credentials for service-owned M2M |
| CAPTCHA on token endpoint           | Blocks automated requests                      | Use rate limiting instead                                                               |
| Session cookies                     | Expire without a structured signal             | Use Bearer tokens with explicit `exp`                                                   |
| Long-lived broad-scope keys         | Maximum blast radius on leak                   | Short-lived, minimum-scope                                                              |
| Overly broad scopes                 | Excess permissions on every token              | Request only required scopes                                                            |
| Hardcoded secrets                   | Permanent compromise on commit                 | Environment variables only                                                              |
| Per-request token calls             | Multiplies auth load                           | Cache for lifetime, refresh proactively                                                 |
...
Agent auth failures are rarely obvious. A CAPTCHA returns an HTML page, not an error. A cookie session expires, and the agent gets redirected instead of a structured response. A broad-scoped token succeeds silently for months, then a leaked key compromises everything at once. These patterns all fail silently or fail late.

## Using the Wrong Grant for the Acting Identity

Authorization Code with PKCE is the normal user-delegated MCP authorization flow. A person grants a client access during connection setup; subsequent protected requests use access tokens. That consent step is not a failure of agent readiness.

Client Credentials serves a different identity: the service itself. Use it for supported unattended M2M access. It must not replace a shopper's consent or give a service principal implicit access to that shopper's private data.

The failure is returning generic login HTML during a protected API request without the discovery and authorization flow the client supports. Remote MCP clients need discoverable authorization metadata and actionable authentication responses. An `auth.md` registration ceremony is separate and does not replace MCP OAuth requirements.

See [OAuth 2.1 for Agents](/docs/authentication/oauth-for-agents) and the [official MCP authorization overview](https://modelcontextprotocol.io/extensions/auth/overview).
...
`auth.md` OTP claim flows are not the same anti-pattern as browser-only login. They are bounded registration ceremonies that end by issuing or upgrading a programmatic credential. They become an anti-pattern only when OTP or browser state is required on every API call.
...
- [auth.md](/docs/authentication/auth-md) - bounded registration for user-bound credentials
