...
## Authorization Code as the Only API Path

Authorization Code flow requires a browser-mediated consent path. It redirects to a login page, the user clicks "Allow," and the server redirects back with a code. That can be acceptable for explicit user consent or client registration, but it is a poor default for unattended agent API access.
...
If your service only supports Authorization Code for all programmatic access, agents fail at step one.

Fix: support Client Credentials for service-owned machine-to-machine access. If the agent needs user-bound credentials, publish a bounded registration path such as `auth.md` rather than forcing agents through a generic sign-in page.
...
|---|---|---|
| Authorization Code only for API access | Requires browser-mediated consent | Support Client Credentials or bounded registration |
