...
- User-delegated access: Authorization Code with PKCE; Token Exchange where narrower downstream delegation is needed
...
For user-delegated access, use Authorization Code with PKCE: the person grants access through the client-supported consent flow, then protected requests use access tokens. This is the core MCP authorization case; Client Credentials is an optional extension for service identities. A bounded registration flow such as [`auth.md`](/docs/authentication/auth-md) serves a separate registration need and does not replace MCP OAuth. See the [MCP authorization overview](https://modelcontextprotocol.io/extensions/auth/overview).
...
Authorization Code with PKCE is the standard choice for user-delegated MCP access. Consent happens during connection setup; subsequent protected calls use tokens with bounded lifetime, audience and scopes. PKCE binds the authorization request to the code exchange; it is not an extra step in the Client Credentials grant.
