...
Read keys once at startup. Fail fast if missing - a missing-key error at boot is better than an auth error in production.
...
## Test-Mode Keys and Sandbox Environments

An agent building an integration needs to be able to try it without risking production data or a real charge. Give it a separate credential space, not a "be careful" note in the docs:

- **Separate test-mode keys.** Prefix so the scheme is obvious at a glance and both humans and agents can grep for it: `sk_live_...` for production, `sk_test_...` for test mode. A leaked test key can't touch production data.
- **A sandbox environment**, not just a test key against production infrastructure. Point test keys at a separate base URL or a clearly-flagged sandbox tenant so a bug in agent-generated code can't reach real customer records.
- **Seeded data.** A sandbox with no data forces the agent to create its own fixtures before it can test a read path. Seed the sandbox with representative records (a handful of customers, orders, invoices in different states) so an agent's first call succeeds without a setup ritual.
- **Same schema, same errors.** The sandbox must return the same response shapes and error codes as production. A sandbox that silently diverges teaches an agent the wrong contract.

```typescript
const apiKey = process.env.API_KEY; // sk_test_... in sandbox, sk_live_... in production
const baseUrl = apiKey.startsWith("sk_test_")
  ? "https://sandbox.api.example.com"
  : "https://api.example.com";
```

This is what lets an agent go from discovery to a working integration with no human in the loop - sign up, get a test key, build against seeded sandbox data, then swap to a live key. See [OpenAPI for Agents](/docs/api-surface/openapi-for-agents#publishing-sdks-and-removing-onboarding-friction) for the rest of that onboarding path.
...
- [ ] Test-mode keys (`sk_test_...`) are distinguishable from live keys at a glance
- [ ] A sandbox environment with seeded data exists so agents can build without touching production
...
- [OpenAPI for Agents](/docs/api-surface/openapi-for-agents#publishing-sdks-and-removing-onboarding-friction) - the rest of the zero-human onboarding path
