...
| A2A v1.0 | Agent → agent | Production | Multi-agent delegation |
| ACP emerging | Agent → merchant | Emerging | Agent initiates purchases |
...
| **A2A v1.0** | Agent-to-agent delegation | Horizontal (peers) | JSON-RPC over HTTPS | OAuth 2.1 + DPoP | `/.well-known/agent.json` | Production |
| **ACP emerging** | Agent-initiated commerce | Horizontal (agent → merchant) | HTTPS REST with SSE | Bearer token + request signing | `/.well-known/acp.json` | Emerging |
...
### A2A v1.0 — Send This Task to the Specialist
...
ACP is a newer open commerce standard. It is appropriate for pilots and controlled production deployments where agent-initiated purchases need explicit payment boundaries.
...
| A2A | Yes | Stable spec, broad cloud support, and 150+ supporting organizations |
| ACP | Controlled deployments | Emerging standard, merchant support and tooling still maturing |
...
- [A2A v1.0](https://a2a-protocol.org/latest/specification/)
