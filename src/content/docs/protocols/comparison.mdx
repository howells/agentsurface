...
| ------------------ | ----------------------------- | ----------------------------- | ------------------------------- | ------------------------------- | ----------------------------- | ---------- |
| **MCP 2026-07-28** | Agent-to-tool / agent-to-data | Vertical (down)               | stdio, Streamable HTTP          | OAuth 2.1 (HTTP only)           | `server/discover`, SDK config | Production |
...
MCP answers the question: what can this agent do? It connects an agent to tools (functions it can call), resources (data it can read), prompts (structured workflows), and tasks for long-running work. The connection is vertical — the agent reaches down into tools and data sources. `2026-07-28` is the newest revision (release candidate locked 2026-05-21, final publication scheduled for 2026-07-28); `2025-11-25` is its predecessor and the revision with the fullest deployed SDK support during the rollout window.
...
| ---------------- | --------------------- | --------------------------------------------------------------------------------------------------- |
| MCP `2026-07-28` | Yes, via `2025-11-25` | Newest revision; ship on `2025-11-25` until SDK support catches up (see [MCP](/docs/protocols/mcp)) |
...
- [MCP 2025-11-25](https://modelcontextprotocol.io/specification/2025-11-25/) and the [2026-07-28 changelog](https://modelcontextprotocol.io/specification/draft/changelog)
