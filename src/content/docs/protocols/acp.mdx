...
description: Open commerce standard from Stripe, OpenAI, and Meta for transactions between AI agents, buyers, and merchants
---

## Summary

ACP defines how agents complete purchases on behalf of users with explicit payment delegation boundaries. Merchants publish capabilities at `/.well-known/acp.json`; agents and merchants negotiate what is available before attempting any payment. It is an open standard created by Stripe, OpenAI, and Meta; current public spec assets live under `spec/2026-04-17`.
...
- New and emerging; evaluate merchant support, tooling, and payment-risk controls before rollout

ACP defines how an AI agent completes a purchase on behalf of a user. It is an open standard from Stripe, OpenAI, and Meta, covering discovery, capability negotiation, payment delegation, and completion — from the first discovery request through the final receipt.
...
ACP is appropriate for controlled pilots and production deployments where agent-initiated commerce requires explicit payment boundaries and audit trails. Treat it as emerging: for mission-critical payments, evaluate current merchant support, tooling maturity, and your risk controls carefully.
