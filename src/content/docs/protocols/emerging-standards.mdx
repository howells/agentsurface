...
OpenTelemetry's GenAI semantic conventions standardize how to instrument LLM calls and agentic workflows, but the current OpenTelemetry docs mark them Development. Metrics include request model, token counts, input/output content, system instructions, and tool call traces. Adoption by Traceloop, Langfuse, Datadog, and others makes this a practical observability baseline, but pin emitted versions and expect migration work before stable.
...
**Adopt production-useful standards now.** Use AGENTS.md, llms.txt, schema.org JSON-LD, and Arazzo workflows. Use OpenTelemetry GenAI semconv for observability, but track emitted convention versions while the spec remains Development.
