...
- **Distribution**: Users can connect your service from AI clients that support remote MCP servers.

---

## Why Ship MCP Publicly?

1. **Distribution**: Users can connect your service from the AI clients they already use.
...
4. **Low friction**: A remote MCP connection can be faster than building and maintaining a separate integration for each client.
...
export async function getMcpServer(userId?: string, teamId?: string) {
  ...
  const scopes = userId && teamId ? await getUserScopes(userId, teamId) : ["api:read"]; // Anonymous clients get read-only access
