...
export async function ensureToolIndex(ctx: McpContext) {
  ...
  const index = await createToolIndex(toolDefinitions, {
    ...
    // Define related-tool groups for discovery
    relatedTools: {
      customer_create: ["customer_get", "customer_list"],
      order_create: ["customer_get", "product_list"],
      invoice_create: ["customer_list", "order_list"],
    },
  });
